...
class ClientesTable(tables.Table):
    ...
    opcoes = tables.TemplateColumn(
        template_name='clientes/botao_acoes_clientes.html', verbose_name='Opções', orderable=False)
...
class AutorizadoTable(tables.Table):
    ...
    opcoes = tables.TemplateColumn(
        template_name='autorizados/botao_acoes_autorizados.html', verbose_name='Opções', orderable=False)
...
class FiadosTable(tables.Table):
    select = tables.CheckBoxColumn(accessor='pk', attrs={'th__input': {
                                   'id': 'select-all'}, 'td__input': {'class': 'row-select'}})
    cliente = tables.Column(verbose_name='Cliente', orderable=False)
    cliente_usuario = tables.Column(
        verbose_name='Autorizado do cliente', orderable=False)
    valor = tables.Column(verbose_name='Valor', orderable=False)
    datadoc = tables.DateColumn(
        verbose_name='Data do lançamento', orderable=False)
    ...
    opcoes = tables.TemplateColumn(
        template_name='fiados/botao_acoes_fiados.html', verbose_name='Opções', orderable=False)

    class Meta:
        model = models.Fiado
        template_name = "django_tables2/bootstrap4.html"
        attrs = {'class': 'table table-bordered table-hover'}
        fields = ('select', 'cliente', 'cliente_usuario',
                  'valor', 'datadoc', 'is_pago')


class OperadorTable(tables.Table):
    nome = tables.Column(verbose_name='Nome', orderable=False)
    opcoes = tables.TemplateColumn(
        template_name='operadores/botao_acoes_operadores.html', verbose_name='Opções', orderable=False)
...
class ProdutosTable(tables.Table):
    ...
    valor = tables.Column(verbose_name='Valor', orderable=False)
    opcoes = tables.TemplateColumn(
        template_name='produtos/botao_acoes_produtos.html', verbose_name='Opções', orderable=False)
...
class CaixaTable(tables.Table):
    ...
    qtd_dinheiro = tables.Column(
        verbose_name='Valor em dinheiro', orderable=False)
    qtd_cd = tables.Column(
        verbose_name='Valor cartão de débito', orderable=False)
...
class CategoriaEntradaTable(tables.Table):
    ...
    opcoes = tables.TemplateColumn(
        template_name='categorias/botao_acoes_categoria_entradas.html', verbose_name='Opções', orderable=False)
...
class CategoriaDespesaTable(tables.Table):
    nome = tables.Column(verbose_name='Categoria', orderable=False)
    opcoes = tables.TemplateColumn(
        template_name='categorias/botao_acoes_categoria_despesas.html', verbose_name='Opções', orderable=False)
...
class DespesasTable(tables.Table):
    ...
    opcoes = tables.TemplateColumn(
        template_name='despesas/botao_acoes_despesas.html', verbose_name='Opções', orderable=False)
...
class EntradasTable(tables.Table):
    ...
    data = tables.Column(verbose_name='Data', orderable=False)
    opcoes = tables.TemplateColumn(
        template_name='entradas/botao_acoes_entradas.html', verbose_name='Opções', orderable=False)
...
class OperadorTotalTable(tables.Table):
    nome = tables.Column(verbose_name="Operador", orderable=False)
    total_vendas = tables.Column(verbose_name="Total Vendido", orderable=False)
    ...
    def render_total_vendas(self, value):
        formatted = f"{value:.2f}"
        return formatted.replace('.', ',')